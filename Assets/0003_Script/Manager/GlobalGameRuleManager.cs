...
public class GlobalGameRuleManager : MonoBehaviour
{
    ...
    [Header("결과별 밀려나는 힘")]
    [SerializeField] private float evenRepelForce = 5f; // 비등 시 양측 동일 힘
    [SerializeField] private float advantageRepelForce = 4f; // 우세 측이 받는 힘
    [SerializeField] private float disadvantageRepelForce = 7f; // 열세 측이 받는 힘
    [SerializeField] private float hitRepelForce = 4f; // 적중 측이 받는 힘
    [SerializeField] private float damagedRepelForce = 8f; // 피격 측이 받는 힘
...
public int GetBonusPowerRateFromSpeed(int selfSpeed, int otherSpeed) // 기존 호출 호환용 메서드
{
    ...
    public float GetRepelForceByResult(DuelResultType resultType) // 결과에 따른 밀려나는 힘 반환
    {
        switch (resultType)
        {
            case DuelResultType.Hit:
                return hitRepelForce; // 적중 힘 반환

            case DuelResultType.Advantage:
                return advantageRepelForce; // 우세 힘 반환

            case DuelResultType.Even:
                return evenRepelForce; // 비등 힘 반환

            case DuelResultType.Disadvantage:
                return disadvantageRepelForce; // 열세 힘 반환

            case DuelResultType.Damaged:
                return damagedRepelForce; // 피격 힘 반환
        }
