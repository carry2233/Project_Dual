...
private void TryStartDuel() // 상호 타겟 상태면 결투 시작, 후딜 상태 대상은 예외 허용
{
    ...
    {
        SetCurrentTarget(null); // 결투 불가 대상이면 타겟 제거
        return;
    }
    ...
    {
        return; // 상대가 넉백 상태면 결투 시작 금지
    }

    bool isTargetSandbag = currentTarget.IsSandbagTargetCharacter(); // 현재 타겟이 샌드백 대상인지 확인
    bool isTargetPostRecovery = currentTarget.IsPostDuelRecoveryStateWithoutExternalForce; // 상대가 넉백이 끝난 후 후딜 상태인지 확인

    if (!isTargetSandbag)
    {
        if (!currentTarget.CanBeDuelTriggeredByOther)
        {
            return; // 상대가 현재 결투를 받을 수 없는 상태면 시작 금지
        }

        if (!isTargetPostRecovery && !currentTarget.IsTargeting(this))
        {
            return; // 일반 상태 대상은 서로 타겟팅 중이어야만 결투 시작 가능
        }
    }
...
private void TryResolveDuel() // 결투 판정 처리
{
    ...
    if (resolvedTarget.IsSandbagTargetCharacter())
    {
        ...
        ApplyResolvedDuelOutcome(selfResult); // 자신 결과 즉시 적용
        resolvedTarget.ApplyResolvedDuelOutcome(otherResult); // 상대 결과 즉시 적용
    ...
    ApplyResolvedDuelOutcome(selfResultNormal); // 자신 결과 즉시 적용
    resolvedTarget.ApplyResolvedDuelOutcome(otherResultNormal); // 상대 결과 즉시 적용
...
private void ApplyResolvedDuelOutcome(GlobalGameRuleManager.DuelResultType resultType) // 최신 결투 결과를 현재 캐릭터에게 즉시 적용
{
