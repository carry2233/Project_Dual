...
private void TryResolveDuel() // 결투 판정 처리
{
    ...
    if (resolvedTarget.IsSandbagTargetCharacter())
    {
        ...
        resolvedTarget.PlayCurrentDuelResolveAnimation(); // 샌드백 대상도 판정 애니메이션은 정상 재생
        ...
        {
            targetActionSound.PlayDuelResultSound(otherResult); // 샌드백 대상의 피격 사운드 재생
        }
    ...
    if (characterActionSound != null)
    {
        characterActionSound.PlayDuelResultSound(selfResultNormal); // 자신의 결투 결과 사운드 재생
    }

    CharacterActionSound targetActionSoundNormal = resolvedTarget.GetCharacterActionSound(); // 상대 행동 사운드 참조 가져오기

    if (targetActionSoundNormal != null)
    {
        targetActionSoundNormal.PlayDuelResultSound(otherResultNormal); // 상대의 결투 결과 사운드 재생
    }
...
private void UpdatePostDuelRecoveryState() // 결투 후 후딜 시작/진행 처리
{
    ...
    if (isWaitingForPostDuelRecoveryStart)
    {
        ...
        currentPostDuelRecoveryTimer = Mathf.Max(0f, postDuelRecoveryDuration); // 후딜 시간 설정
