...
private void TryStartDuel() // 상호 타겟 상태면 결투 시작, 샌드백 대상은 예외 허용
{
    ...
    bool isTargetSandbag = currentTarget.IsSandbagTargetCharacter(); // 현재 타겟이 샌드백 대상인지 확인

    if (!isTargetSandbag && !currentTarget.IsTargeting(this))
    {
        return; // 일반 대상은 서로 타겟팅 중이어야만 결투 시작 가능
    }
    ...
    BeginIgnoreCollisionWithCurrentDuelTarget(); // 돌진 시작 시 현재 결투 상대와 물리 접촉 무시 시작
    ...
    PlayCurrentDuelDashSound(); // 현재 결투 기술의 돌진 시작 사운드 재생
}
...
private void TryResolveDuel() // 결투 판정 처리
{
    ...
    if (resolvedTarget.IsSandbagTargetCharacter())
    {
        ...
        PlayCurrentDuelResolveAnimation(selfResult); // 자신의 결투 결과별 판정 애니메이션 재생
        resolvedTarget.PlayCurrentDuelResolveAnimation(otherResult); // 샌드백 대상의 결과별 판정 애니메이션 재생

        PlayCurrentDuelResolveSound(selfResult); // 자신의 결투 결과별 사운드 재생
        resolvedTarget.PlayCurrentDuelResolveSound(otherResult); // 샌드백 대상의 결과별 사운드 재생

        ApplyRepelForce(resolvedTarget, globalGameRuleManager, selfResult, otherResult); // 기존 넉백 규칙 그대로 적용

        EndDuel(false); // 자신의 결투 종료, 판정 애니메이션은 유지
        resolvedTarget.ReceiveResolvedDuel(otherResult); // 샌드백 대상 종료 처리
    ...
    PlayCurrentDuelResolveAnimation(selfResultNormal); // 자신의 결투 결과별 판정 애니메이션 재생
    resolvedTarget.PlayCurrentDuelResolveAnimation(otherResultNormal); // 상대의 결과별 판정 애니메이션 재생

    PlayCurrentDuelResolveSound(selfResultNormal); // 자신의 결투 결과별 사운드 재생
    resolvedTarget.PlayCurrentDuelResolveSound(otherResultNormal); // 상대의 결투 결과별 사운드 재생
    ...
    LockCurrentFacingDirectionAfterDuel(); // 자신의 현재 방향 고정 시작
    resolvedTarget.LockCurrentFacingDirectionAfterDuel(); // 상대의 현재 방향 고정 시작

    StartPostDuelStopTimer(); // 자신의 결투 후 정지 시간 카운트 시작
    resolvedTarget.StartPostDuelStopTimer(); // 상대의 결투 후 정지 시간 카운트 시작
    PreparePostDuelRecovery(); // 자신의 결투 후 후딜 대기 시작
    resolvedTarget.PreparePostDuelRecovery(); // 상대의 결투 후 후딜 대기 시작

    ApplyRepelForce(resolvedTarget, globalGameRuleManager, selfResultNormal, otherResultNormal); // 밀려나는 힘 적용

    EndDuel(false); // 자신의 결투 종료, 판정 애니메이션은 유지
    resolvedTarget.ReceiveResolvedDuel(otherResultNormal); // 저장된 상대 참조로 결과 전달
}
...
private void OnDisable() // 비활성화 시 참조 정리
{
    ...
    if (characterAnimationPlayer != null)
    {
        characterAnimationPlayer.SetImageFrozen(false); // 비활성화 시 이미지 고정 해제
    }
...
private void UpdateDuelTargetCollisionIgnoreTimer() // 결투 상대와의 물리 접촉 무시 시간 갱신
{
    ...
    currentDuelTargetCollisionIgnoreTimer -= Time.deltaTime; // 남은 충돌 무시 시간 감소
...
private void UpdatePostDuelRecoveryState() // 결투 후 후딜 시작/진행 처리
{
    ...
    if (isWaitingForPostDuelRecoveryStart)
    {
        ...
        characterAnimationPlayer?.SetImageFrozen(true); // 현재 이미지 고정
    }
    ...
    currentPostDuelRecoveryTimer -= Time.deltaTime; // 후딜 시간 감소
    ...
    characterAnimationPlayer?.SetImageFrozen(false); // 이미지 고정 해제
...
private void UpdatePostDuelStopTimer() // 결투 후 정지 시간 감소 처리
{
    ...
    currentPostDuelStopTimer -= Time.deltaTime; // 남은 시간 감소
