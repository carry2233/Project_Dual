...
[Header("플레이어 시작 타일 소속 설정")]
[SerializeField] private PlayerTileMembership playerTileMembership; // 시작 타일 정보를 넘길 플레이어 소속 스크립트

private GameObject lastSpawnedStartPointTileObject; // 마지막으로 생성된 시작점 타일 오브젝트
...
public void PlacePrefabsInRange() // 현재 설정값 기준으로 범위 내 타일 생성 실행
{
    ...
    lastSpawnedStartPointTileObject = null; // 시작점 타일 참조 초기화
...
private void SpawnPrefabAtCell(Vector3Int cellPosition, GameObject prefabToSpawn) // 지정한 셀 중심 위치에 원하는 프리팹 생성
{
    ...
    {
        lastSpawnedStartPointTileObject = spawnedObject; // 시작점 타일이면 참조 저장
    }
...
private void RefreshStartPointReferenceFromPlacedObjects() // 현재 배치된 오브젝트 중 시작점 타일 재탐색
{
    if (startPointTilePrefab == null)
    {
        return; // 시작점 프리팹이 없으면 종료
    ...
    TilePrefab startPrefabInfo = startPointTilePrefab.GetComponent<TilePrefab>(); // 시작점 프리팹 정보 가져오기
    if (startPrefabInfo == null)
    {
        return; // TilePrefab 정보가 없으면 종료
    }

    foreach (KeyValuePair<Vector3Int, GameObject> pair in placedObjectMap) // 배치된 오브젝트 전체 순회
    {
        if (pair.Value == null)
        {
        ...
        TilePrefab placedTilePrefab = pair.Value.GetComponent<TilePrefab>(); // 현재 배치 타일 정보 가져오기
        if (placedTilePrefab == null)
        {
            continue; // 타일 정보가 없으면 건너뜀
        }

        if (placedTilePrefab.TilePrefabNumber == startPrefabInfo.TilePrefabNumber)
        {
            lastSpawnedStartPointTileObject = pair.Value; // 시작점 타일 참조 갱신
            return;
        }
    }
}

private void ApplyPlayerStartTileMembership() // 시작점 타일 정보를 플레이어 소속 정보에 반영
{
    ...
    if (lastSpawnedStartPointTileObject == null)
    {
        playerTileMembership.ClearCurrentTile(); // 시작점 타일이 없으면 정보 초기화
        return;
    }

    TilePrefab startTilePrefab = lastSpawnedStartPointTileObject.GetComponent<TilePrefab>(); // 시작점 타일 정보 가져오기
    if (startTilePrefab == null)
    {
        playerTileMembership.ClearCurrentTile(); // 정보가 없으면 초기화
        return;
    }
