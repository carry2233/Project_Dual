...
[Header("NavMesh 이동 가능 영역 참조")]
[SerializeField] private NavMeshSurface targetNavMeshSurface; // 이동 가능 영역 판정에 사용할 NavMeshSurface 참조

[Header("NavMesh 이동 가능 영역 검사 설정")]
[SerializeField] private bool allowMoveOnlyOnBakedNavMesh = true; // 베이크된 NavMesh 영역에서만 이동 명령 허용 여부
[SerializeField] private float navMeshSampleMaxDistance = 0.2f; // 클릭 위치 기준 NavMesh 샘플링 최대 허용 거리
...
private void Awake() // 시작 시 카메라 자동 참조 및 표시 오브젝트 초기값 준비
{
    ...
    if (targetNavMeshSurface == null)
    {
        targetNavMeshSurface = FindFirstObjectByType<NavMeshSurface>(); // NavMeshSurface 자동 탐색
    }
...
private void HandleMoveInput() // 목적지 입력 또는 공격 대상 입력 처리
{
    ...
    if (!CanIssueMoveOrAttackCommandToCurrentSelection())
    {
    ...
    if (!IsMovePositionAllowedOnNavMesh(mouseWorldPosition))
    {
        return; // 베이크된 NavMesh 영역이 아니면 이동 명령 차단
    }
...
private bool CanIssueMoveOrAttackCommandToCurrentSelection() // 현재 선택 유닛이 실제 아군 목록 등록 대상인지 확인
{
    if (currentSelectedUnit == null)
    {
        return false; // 선택 유닛이 없으면 false
    }

    if (friendlyCharacterManager == null)
    {
        return false; // 아군 매니저가 없으면 false
    }

    return friendlyCharacterManager.IsRegisteredFriendlyMoveCommandController(currentSelectedUnit); // 실제 아군 목록 등록 컨트롤러인지 반환
}

private bool IsMovePositionAllowedOnNavMesh(Vector2 targetPosition) // 클릭 위치가 베이크된 NavMesh 영역인지 확인
{
    if (!allowMoveOnlyOnBakedNavMesh)
    {
        return true; // NavMesh 제한을 사용하지 않으면 항상 허용
    }

    if (targetNavMeshSurface == null)
    {
        return false; // NavMeshSurface 참조가 없으면 안전하게 차단
    }

    NavMeshHit navMeshHit; // 샘플링 결과 저장용

    bool foundNavMesh = NavMesh.SamplePosition(
        new Vector3(targetPosition.x, targetPosition.y, 0f), // 클릭 위치 기준 샘플링
        out navMeshHit, // 샘플링 결과 반환
        navMeshSampleMaxDistance, // 허용 거리
        NavMesh.AllAreas); // 전체 NavMesh 영역 검사

    if (!foundNavMesh)
    {
        return false; // 근처에 NavMesh가 없으면 차단
    }

    Vector2 sampledPosition = new Vector2(navMeshHit.position.x, navMeshHit.position.y); // 샘플링된 NavMesh 위치
    float distanceToClickedPoint = Vector2.Distance(targetPosition, sampledPosition); // 클릭 위치와 샘플링 위치 차이 계산
